...
# 生成简单数列数据
def generate_sequences(num_sequences=1000, seq_length=10):
    """生成简单数列数据，例如等差数列、等比数列等"""
    sequences = []
    targets = []
    
    for _ in range(num_sequences):
        # 随机选择数列类型
        seq_type = np.random.choice(['arithmetic', 'geometric', 'quadratic'])
        ...
        if seq_type == 'arithmetic':
            ...
            ratio = np.random.uniform(1.1, 2.0)
            seq = [start * (ratio ** i) for i in range(seq_length)]
            
        else:  # quadratic
            # 二次数列: a_n = a*n^2 + b*n + c
            a = np.random.randint(1, 3)
            b = np.random.randint(1, 5)
            c = np.random.randint(1, 5)
            seq = [a * i**2 + b * i + c for i in range(seq_length)]
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=100, patience=10):
    ...
    for epoch in range(num_epochs):
        ...
        for sequences, targets in train_loader:
            ...
            tolerance = 0.05 * np.abs(actual) + 0.05  # 5%的相对误差加上0.05的绝对误差
        ...
        with torch.no_grad():
            for sequences, targets in val_loader:
                ...
                predicted = outputs.detach().cpu().numpy()
                actual = targets.detach().cpu().numpy()
                tolerance = 0.05 * np.abs(actual) + 0.05
                correct += np.sum(np.abs(predicted - actual) < tolerance)
                total += len(actual)
...
# 生成数据
sequences, targets = generate_sequences(num_sequences=5000, seq_length=seq_length+1)
...
optimizer = optim.AdamW(model.parameters(), lr=learning_rate, weight_decay=1e-5)  # 使用AdamW优化器
