...
class OptimizedTransformerModel(nn.Module):
    def __init__(self, input_dim, d_model, nhead, num_layers, dim_feedforward=512, dropout=0.1):
...
# 分析数列类型
def analyze_sequence_type(sequence):
    """分析输入的数列类型"""
    if len(sequence) < 3:
        return "unknown"
    ...
    # 判断数列类型
    if all(abs(d - diff1[0]) < 1e-6 for d in diff1):
        return "arithmetic"  # 等差数列
    elif len(ratios) > 0 and all(abs(r - ratios[0]) < 1e-6 for r in ratios):
        return "geometric"  # 等比数列
    elif all(abs(d - diff2[0]) < 1e-6 for d in diff2):
        return "quadratic"  # 二次数列
    else:
        return "complex"  # 复杂数列

# 生成数列的多个预测值
def predict_multiple_numbers(model, sequence, num_predictions=5):
    """使用模型预测数列的多个后续数字"""
    predictions = []
    ...
    for _ in range(num_predictions):
        # 预测下一个数字
        next_num = predict_next_number(model, current_sequence)
        predictions.append(next_num)
        
        # 将预测的数字添加到序列中，用于下一次预测
        current_sequence.append(next_num)
        
        # 保持序列长度不超过9
        if len(current_sequence) > 9:
            current_sequence = current_sequence[-9:]
...
def main():
    print("=== 复杂数列预测交互程序 ===")
    print("本程序使用优化版Transformer模型预测数列的下一个数字。")
    print("模型支持等差数列、等比数列、二次数列和复杂数列的预测。")
    ...
    try:
        ...
        print("请确保已运行 'sequence_predictor.py' 生成模型文件。")
        return
    ...
    while True:
        ...
        if user_input.lower() in ['help']:
            print("\n=== 帮助信息 ===")
            print("1. 输入数列：用逗号分隔的数字序列，例如：1, 3, 5, 7, 9")
            print("2. 预测结果：程序将预测数列的下一个数字")
            print("3. 数列分析：程序会分析输入的数列类型（等差、等比、二次或复杂）")
            print("4. 多步预测：可以预测数列的多个后续数字")
            print("5. 退出程序：输入 'quit' 或 'exit'")
            print()
            continue
        ...
        # 分析数列类型
        seq_type = analyze_sequence_type(sequence)
        type_names = {
            "arithmetic": "等差数列",
            "geometric": "等比数列",
            "quadratic": "二次数列",
        ...
        print(f"数列类型分析：{type_names.get(seq_type, '未知类型')}")
        ...
        try:
            ...
            # 询问是否需要多步预测
            multi_pred = input("是否需要预测多个后续数字？(y/n): ").strip().lower()
            if multi_pred in ['y', 'yes']:
                num_predictions = int(input("请输入要预测的数字个数："))
                if num_predictions > 0:
                    multiple_predictions = predict_multiple_numbers(model, sequence, num_predictions)
                    print(f"预测的后续{num_predictions}个数字：")
                    for i, pred in enumerate(multiple_predictions):
                        print(f"  第{i+1}个数字：{pred:.2f}")
